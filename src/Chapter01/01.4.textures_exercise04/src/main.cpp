...
int main()
{
    ...
    Shader shader("../shaders/01.4.textures_exercise04.vert", "../shaders/01.4.textures_exercise04.frag");
...
void processInput(GLFWwindow *window)
{
    ...
    if(glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        mixFactor += 0.01f;
        mixFactor = mixFactor > 1.0f ? 1.0f : mixFactor;
    }
    
    if(glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
        mixFactor -= 0.01f;
        mixFactor = mixFactor < 0.0f ? 0.0f : mixFactor;
    }
}
