...
int main() {
    ...
    Shader shader("../shaders/04.5_exercise01.framebuffers.vert", "../shaders/04.5_exercise01.framebuffers.frag");
    Shader postProcessShader("../shaders/04.5_exercise01.framebuffer_postprocesspass.vert", "../shaders/04.5_exercise01.framebuffer_postprocesspass.frag");
    ...
    unsigned int rearViewWidth = SCR_WIDTH;
    unsigned int rearViewHeight = SCR_HEIGHT;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, rearViewWidth, rearViewHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, rearViewWidth, rearViewHeight);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Rear View Rendering
        shader.use();
        glm::mat4 rearViewMatrix = camera.getOppositeViewMatrix();
        shader.setMat4f("view", rearViewMatrix);
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
        glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        // ----------------------------------------------------------------------------------------------------
        // Draw Scene in the rear view mirror.
        // ----------------------------------------------------------------------------------------------------
        // Cubes
        glBindVertexArray(cubeVAO);
        containerTex.bindToTextureUnit(0);
