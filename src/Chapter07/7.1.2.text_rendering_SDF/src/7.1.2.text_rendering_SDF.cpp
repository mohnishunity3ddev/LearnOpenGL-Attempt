...
int main() {
    ...
    Shader shader("../shaders/ui_sdf.vert", "../shaders/ui_sdf.frag");
    ...
    // Make the sdf texture 
    Sdf sdf("Ubuntu Mono.ttf", 32, 512, '#');

    // Get the sdf texture loaded in a separate texture to be used for sampling
    // in the shader.
    Texture fontTex("_testImages/fontTexture.png");
    fontTex.bindToTextureUnit(0);
    shader.use();
    shader.setInt("fontTexture", 0);
    shader.setVec3("fontColor", glm::value_ptr(yellow));
    shader.setVec3("outlineColor", glm::value_ptr(red));
